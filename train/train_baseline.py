...
def get_args():
    ...
    parser.add_argument(
        "--validation", "-v", dest="val", type=float, default=10.0, help="Percent of the data that is used as validation (0-100)"
    )
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_score > best_val_score:
            ...
            patience_counter = 0
            # 可选：如果只想保存最好的模型，可以在这里把 save_checkpoint 设为 True，否则设为 False
        else:
            patience_counter += 1
...
if __name__ == "__main__":
    ...
    if args.load:
        # [修改] 添加 weights_only=True 以消除 FutureWarning (可选，如报错可改回 False)
